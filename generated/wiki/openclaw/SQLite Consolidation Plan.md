...
- `business_ledger`: 438 DBs. Business ledger and ledger-shaped databases are protected; consolidation risk is forbidden.
...
- `package_queue_event_concepts` (medium): Create a read-only package_event_overlay view/index plan over package queue and package event index refs; do not migrate rows.
...
- `work_log_staging_if_safe` (medium): Keep staged work logs isolated until operator confirmation; future overlay may expose package/work-log refs without workbook mutation.

## Recommended First Low-Risk Move

Create views/indexes over existing package/event/journal refs, not a data migration. This remains plan-only here.
