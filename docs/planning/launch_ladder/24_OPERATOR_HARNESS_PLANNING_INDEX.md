...
17. `docs/planning/launch_ladder/source_set_bridges/sqlite_schema_definition_project_source_set_20260506.md` before creating or refreshing the `05_BACKEND_SQLITE_SCHEMA_ORCHESTRATION` ChatGPT Project packet.
18. `docs/planning/project_packets/05_BACKEND_SQLITE_SCHEMA_ORCHESTRATION/README.md` after the source-set guide when the operator needs the PC-side ChatGPT Project upload packet and Mac mirror sync target.
19. `docs/planning/launch_ladder/knowledge_substrate/02_SQLITE_LAYER_MODEL.md` if present, or the imported equivalent if differently named.
20. `docs/planning/launch_ladder/29_WINDOWS_ROOT_DEPENDENCY_MAP.md` as exclusion/classification guidance only before `04_BACKEND_DATA_CONTRACT_READINESS` source-set generation.
21. `docs/planning/launch_ladder/30_BACKEND_SOURCE_SET_BRIDGE_AND_EXCLUSION_PLAN.md` as the compact bridge that keeps dependency-map facts out of private-root content and backend authority.
