...
- [Windows Root Triage / Dependency Map Plan](./planning/launch_ladder/27_WINDOWS_ROOT_TRIAGE_AND_DEPENDENCY_MAP_PLAN.md) — Metadata-only sequencing for Windows root dependency mapping; read with the [PC Windows roots boundary](./planning/launch_ladder/26_PC_WINDOWS_ROOTS_PRIVATE_DATA_BOUNDARY_BREADCRUMB.md) and [dependency-map template](./planning/launch_ladder/28_WINDOWS_ROOT_DEPENDENCY_MAP_TEMPLATE.md); not cleanup, migration, source-set, backend, bridge, or ingestion authority.
