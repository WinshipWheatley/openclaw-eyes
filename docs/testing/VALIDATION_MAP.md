...
| `backend_data_contract.py`, `tests/test_backend_data_contract.py` | `tests/test_backend_data_contract.py`, `tests/test_launch_ladder_static_contract.py` | Verifies backend data-contract semantic vocabulary and guard helpers, no-runtime schema-contract surfaces for semantic_record/semantic_label/semantic_relationship/provenance_ref/validation_receipt/operator_promotion/context_filter_receipt, no-runtime SQLite table-concept contracts for semantic_records/semantic_labels/semantic_relationships/provenance_refs/validation_receipts/operator_promotions/context_filter_receipts, field-bundle validator coverage for raw/compiled/wiki/relationship/synthesis/write-back layers, entity-family validator coverage for person/organization/client/job/invoice/payment/project/music/legal/tax/source/compiled/relationship/synthesis/follow-up/approval/blocker/system families, provenance/freshness/confidence/sensitivity/authority/review labels, confirmed/inferred/excluded/unknown-style states, missing-label reasons, write-back label/promotion requirements, synthesis not being accepted knowledge by default, private/sensitive promotion guards, receivables/accountability without automated sending or harassment, and no CLI/DB/SQLite/SQL DDL/migration/persistence/API/MCP/model/provider/ingestion/runtime/frontend/app/private-root-inspection behavior |
| `docs/planning/launch_ladder/source_set_bridges/backend_data_contract_storage_schema_plan_20260505.md` | `git diff --no-index --check -- /dev/null docs/planning/launch_ladder/source_set_bridges/backend_data_contract_storage_schema_plan_20260505.md`, final-byte check, `tests/test_launch_ladder_static_contract.py` | Verifies the docs-only storage/schema planning bridge is whitespace-clean, newline-terminated, discoverable, and reviewed with adjacent Launch Ladder static contracts; the plan must not authorize SQLite, schema migrations, persistence, API routes, ingestion, indexing, embeddings, runtime services, provider/model calls, private-root inspection, fixture generation, or source-set generation |
| `docs/planning/launch_ladder/source_set_bridges/backend_data_contract_sqlite_plan_20260505.md` | `git diff --no-index --check -- /dev/null docs/planning/launch_ladder/source_set_bridges/backend_data_contract_sqlite_plan_20260505.md`, final-byte check, `tests/test_launch_ladder_static_contract.py` | Verifies the docs-only SQLite planning bridge is whitespace-clean, newline-terminated, discoverable, and reviewed with adjacent Launch Ladder static contracts; the plan must not authorize database creation, SQL DDL execution, migrations, persistence, DB connections, file I/O, API routes, ingestion, indexing, embeddings, runtime services, provider/model calls, Hermes, MCPs, sync, private-root/private-data inspection, fixture generation, source-set generation, or app behavior |
| `docs/planning/launch_ladder/source_set_bridges/backend_data_contract_sqlite_implementation_readiness_20260505.md` | `git diff --no-index --check -- /dev/null docs/planning/launch_ladder/source_set_bridges/backend_data_contract_sqlite_implementation_readiness_20260505.md`, final-byte check, `tests/test_launch_ladder_static_contract.py` | Verifies the docs-only SQLite implementation-readiness bridge is whitespace-clean, newline-terminated, discoverable, and reviewed with adjacent Launch Ladder static contracts; the readiness artifact must not authorize SQLite implementation, SQL DDL execution, migrations, persistence, DB connections, file I/O, API routes, ingestion, extraction, indexing, embeddings, fixtures, runtime services, frontend/app behavior, provider/model calls, Hermes, MCPs, sync, source-set generation, private-root/private-data inspection, or app behavior |
| `docs/planning/launch_ladder/source_set_bridges/backend_data_contract_first_sqlite_implementation_plan_20260505.md` | `git diff --no-index --check -- /dev/null docs/planning/launch_ladder/source_set_bridges/backend_data_contract_first_sqlite_implementation_plan_20260505.md`, final-byte check, `tests/test_launch_ladder_static_contract.py` | Verifies the first SQLite implementation planning bridge is whitespace-clean, newline-terminated, discoverable, and reviewed with adjacent Launch Ladder static contracts; the plan may prepare a future `backend_sqlite_schema.py` and `test_backend_sqlite_schema.py` implementation prompt with inert SQL schema constants only, and must not authorize sqlite3, DB connections, SQL DDL execution, migrations, persistence, file I/O runtime behavior, APIs, ingestion, extraction, indexing, embeddings, fixtures, runtime services, frontend/app behavior, provider/model calls, Hermes, MCPs, sync, source-set generation, private-root/private-data inspection, or app behavior |
| `docs/planning/launch_ladder/*.md`, `docs/planning/launch_ladder/fixtures/mission_control/*.json`, `docs/planning/launch_ladder/knowledge_substrate/*.md`, `docs/planning/OPENCLAW_MODULAR_READINESS_LEDGER.md`, `mac_eyes/Launchers/sync_operator_harness_to_mac.sh`, `mac_eyes/Launchers/refresh_operator_harness_ingest.sh`, `launch_ladder_contract_check.py` | `tests/test_launch_ladder_static_contract.py` | Verifies Launch Ladder stages, route compression, compact buttons, Launch Packet minimum fields, Action Authorization / Approval Receipt lifecycle and scope rules, UI State Claim evidence/freshness rules, Product Taste / Operator Experience Eval Spine, golden/malformed taste examples, Mac desktop Mission Control read-only fixture contract and JSON fixture boundaries, first-screen composition zones/fixtures/app-naming boundary/local-ahead-not-synced rule, Mac desktop taste/atmosphere spec, separate sound/haptics quiet feedback addendum, no audio-assets/haptics/notification/sound-settings implementation boundary, SQLite-backed Compiled Knowledge Substrate planning package and synthetic-only no-ingestion/no-external-model-sensitive-data boundaries, `03_MAC_APP_KNOWLEDGE_SUBSTRATE` source-set brief and 23+MANIFEST=24 generation contract, `17_BACKEND_DATA_CONTRACT_READINESS_PLAN.md` and `04_BACKEND_DATA_CONTRACT_READINESS` readiness-only next-source-set plan, backend/data-contract record topics, `18_BACKEND_DATA_CONTRACT_SHAPE_PLAN.md` backend data-contract shape plan conceptual records and relationship rules, backend semantic contract matrix, backend implementation-readiness checklist, backend first implementation slice readiness, source-set 04 context-filter freshness bridge, doc 30 exclusion/classification-only handling, `19_STORAGE_AND_SOURCE_REGISTRY_READINESS_PLAN.md` storage and source registry readiness plan, `20_PC_STORAGE_RELIEF_LAUNCH_PACKET_PLAN.md` PC storage relief launch-packet plan, no backend/schema/SQLite/ingestion/fixture/private/provider/runtime/approval/app-naming/audio-haptic-notification/source-set-generation authorization, profile-to-packet handoff boundaries, parallel bundle requirements, view modes, evidence/freshness fields, generated `MANIFEST.md` upload authority, Source-Set Ladder and adjacent `CHAT_STAY_UP_TO_DATE.md` bridge behavior, Workspace Launch Profile record shape/example fixtures/navigation-only boundaries, current source-set posture, Apple-platform build-order clarification, prototype `.claude` bridge non-authority/retirement notes, non-authority boundaries, Atlas horizon/zoom levels, and active app-planning source-set freshness warnings |
