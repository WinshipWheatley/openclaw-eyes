...
| `backend_data_contract.py`, `tests/test_backend_data_contract.py` | `tests/test_backend_data_contract.py`, `tests/test_launch_ladder_static_contract.py` | Verifies backend data-contract semantic vocabulary and guard helpers, field-bundle validator coverage for raw/compiled/wiki/relationship/synthesis/write-back layers, provenance/freshness/confidence/sensitivity/authority/review labels, confirmed/inferred/excluded/unknown-style states, missing-label reasons, write-back label/promotion requirements, synthesis not being accepted knowledge by default, private/sensitive promotion guards, and no CLI/DB/SQLite/API/MCP/model/provider/ingestion/runtime/frontend/app behavior |
