...
8. Operator Next Sane Thing (Deferred PI Concept)
   - Status: DEFERRED / NOT ACTIVE / NOT STANDALONE.
   - Candidate policy: The concept previously referred to as PI (Personal Intelligence) is deferred. It must not become a standalone autonomous agent or lane.
   - Candidate policy: The useful safe shape is a deterministic `Operator Next Sane Thing` packet that helps the operator reduce ambiguity. This is a UX/packet pattern to be absorbed into existing agents (e.g., Chief or Cassandra) where useful, not implemented as a separate agent.
...
## Progressive Discovery Contract -- 2026-04-30

Status: INFERRED FROM CODE/DOCS for cross-lane policy shape; CONFIRMED BY CODE/DOCS where noted by existing helpers, brokers, harnesses, and packet builders.
...
Implemented MCP hardening note: `.mcp.json` now defaults filesystem MCP access to `docs/operations` and `docs/specs` only. Repo source, shared vaults, logs, Legal private paths, Hermes runtime state, providers, messaging, terminal/process tools, write tools, and external MCP/plugin discovery remain withheld unless an explicit unlock or trusted-local-development profile is selected with its required gate and reveal artifact. This section does not authorize service, scheduler, provider, Gmail, Telegram, Hermes runtime, or live model expansion.
...
|---|---|---|---|---|---|
| Chief morning | CONFIRMED BY CODE/DOCS / [CHIEF_MACHINE_CONTRACT.md](./CHIEF_MACHINE_CONTRACT.md): bounded upstream artifact snapshots and morning synthesis inputs. | Raw logs, full private context, vault contents, service controls, and broad model/tool access. | Scheduled or operator-requested morning run with explicit upstream artifact selection. | Protected-window checks, stage policy, harness replay checks, and bounded synthesis builders. | Morning synthesis Markdown/JSON, stage metadata, archive entry, and harness manifest. |
| Chief EOD | CONFIRMED BY CODE/DOCS / [CHIEF_MACHINE_CONTRACT.md](./CHIEF_MACHINE_CONTRACT.md): compact review context and structured advisory proposal payload. | Full loop logs, direct queue mutation, service controls, and model free-roam over private state. | End-of-day review request after active-user/idle checks permit review. | Active-user guard, structured-output parser, fallback proposal path, and EOD harness evidence contract. | EOD review artifact, advisory proposal record, replay fixture, and harness manifest. |
| Cassandra email triage | BUILT BEHAVIOR / [CASSANDRA_MACHINE_CONTRACT.md](./CASSANDRA_MACHINE_CONTRACT.md): mocked metadata-only candidate and unsent operator display packet. | Gmail bodies, live inbox polling, labels/archive/delete/move, Telegram sends, Gmail drafts/sends, Guardian approvals, private correspondence, and external models. | Operator runs mocked training/demo or later explicitly approves a live metadata-poller design. | Metadata-only helper contract, synthetic tests, no-live-import boundary, and future Google broker gate for live metadata. | Classification JSONL record, unsent operator packet/display, and dry-run demo output. |
| Cassandra known-contact/payment/contact paths | BUILT BEHAVIOR / [CASSANDRA_MACHINE_CONTRACT.md](./CASSANDRA_MACHINE_CONTRACT.md): resolver/decision helpers; live action wiring remains unapproved. | Arbitrary Gmail roam, unrelated threads, Gmail body reads without broker policy, draft creation, sends, Telegram notification, payment claims, and lane expansion. | Known contact/thread candidate plus latest operator action or explicit operator assignment. | Known-contact decision resolver, centralized action/reason constants, fail-closed suppression, Google Class B/C broker gates, and Guardian send approval when applicable. | Known-contact JSONL state, notification decision record, Google broker audit log, and correspondence/action log. |
| Hermes advisory | CONFIRMED BY DOCS for advisory policy; CONFLICT / NEEDS USER DECISION for broad native Hermes capability outside OpenClaw modes. | Canonical writes, private vault/log/session/state DB access, messaging sends, broad terminal/file tools, MCP discovery, and external fallback providers. | Explicit OpenClaw gateway/lab mode or manual proof request with bounded input. | OpenClaw gateway toolset resolution, lab write-root restrictions, file-safety deny rules, manual proof checker, and non-canonical proposal schema. | Checked advisory proof, proposal artifact, denied-write reason, gateway/lab toolset manifest, or proof-check result. |
| Expert escalation | CONFIRMED BY CODE/DOCS: sanitized expert escalation packet. | Sensitive/private source data, provider execution, concrete model selection, queue execution, Hermes execution, and outbound calls. | Operator requests external expert help using a checked sanitized packet. | Packet checker, lane selector, provider policy, queue transition checks, and no-execution job manifest until explicit approval. | Checked packet, provider/lane selection record, queue item, and no-execution job manifest. |
| Operator Next Sane Thing (Deferred PI Concept) | DEFERRED / NOT STANDALONE; Reserved as a deterministic advisory pattern to be absorbed into existing agents. | Execution authority, approvals, queue mutation, memory writes, service actions, actor override, private logs, Legal/CPA/Music Law/Publishing sensitive data, Gmail bodies, Hermes sessions/state DB, vault contents, and raw PII. | Future explicit approval to generate one next-action advisory from bounded lane/status summaries. | Deterministic schema validation, advisory-only boundary note, Chief/Cassandra/Guardian/Hermes authority separation, and no external-model default. | Operator next-action packet, input-scope list, forbidden-actions field, and boundary note. |
...
- Expert handoff chain: `expert_escalation_job_manifest.py` creates no-execution job metadata with `execution_allowed: false` and `approval_required: true`, and its builder now owns canonical `manifest_hash` minting. The provider-plan builder now owns canonical `provider_plan_hash` minting. `expert_execution_approval_receipt.py` validates Guardian-bound approval receipts and requires both canonical hashes. `expert_result_schema.py` validates bounded result artifacts against receipt, manifest, and provider metadata; concrete `model_selected` remains disallowed by default, and private markers plus unsafe artifact paths fail closed.
- Coordination note: manifest and provider-plan hash ownership is now canonical at the builder boundary. Downstream integration must preserve those hashes instead of re-minting them before any approved live handoff or result acceptance.
...
| Guardian | Human approval bot and high-risk approval path. | CONFIRMED BY CODE/DOCS / [GUARDIAN_MACHINE_CONTRACT.md](./GUARDIAN_MACHINE_CONTRACT.md) | Receive approval prompts, record expected approval IDs, approve/deny/delay/why. | Should not be bypassed for Tier 2 actions. | Tier 2 external/high-risk approval. | Approval prompts may contain sensitive summaries; should avoid secrets. | No model role found. | `/home/openclaw/chief_guardian_listener.py`, `/home/openclaw/chief_guardian_sender.py`, `/home/openclaw/docs/operations/GUARDIAN_MACHINE_CONTRACT.md` | No |
| Hermes | Advisory sidecar, non-canonical synthesis/tooling gateway. | CONFLICT / NEEDS USER DECISION | Docs allow evidence-bound advisory work. Code allows broad file/terminal/MCP tools. | Docs forbid canonical mutation/governance/approval authority. | Hermes has its own command approval modes; not OpenClaw canonical approval. | No OpenClaw-specific denylist found for LegalPrivate or repo-sensitive paths. | Unclear; Hermes has synthesis/tool surface. | `docs/operations/HERMES_MACHINE_CONTRACT.md`, `/home/openclaw/sidecars/hermes_home/HERMES_ORIENTATION.md`, `/home/openclaw/sidecars/hermes/LANE_POLICY.md`, `/home/openclaw/sidecars/hermes/tools/file_tools.py`, `/home/openclaw/sidecars/hermes/tools/approval.py` | Yes |
| Agent Capability Pattern Inventory | Manual inventory of reusable agent-specific capability patterns. | CONFIRMED BY DOCS | Track discovered patterns; recommend candidate_shared for future review. | Automatic promotion, runtime authority, or direct mutation. | Manual planning pass; future Hermes advisory review. | Patterns should be generalized; no raw PII or secrets. | Advisory only; no direct model role. | `docs/operations/OPENCLAW_AGENT_CAPABILITY_PATTERN_INVENTORY_V0.md` | No |
...
| Hermes file read/write | Generic file tools; limited system-path denylist. | CONFLICT with advisory/non-canonical docs. | Generic denylist only. | OpenClaw-specific deny/allow policy. | Yes | Very high | `docs/operations/HERMES_MACHINE_CONTRACT.md`, `/home/openclaw/sidecars/hermes/tools/file_tools.py`, `/home/openclaw/sidecars/hermes_home/HERMES_ORIENTATION.md` | Yes |
| Hermes terminal command | Approval tool supports manual/smart/off/yolo and environment bypasses. | CONFLICT with advisory intent. | Hermes approval modes, not Chief. | Canonical OpenClaw gate for repo/private paths. | Yes | High | `docs/operations/HERMES_MACHINE_CONTRACT.md`, `/home/openclaw/sidecars/hermes/tools/approval.py` | Yes |
...
| Chief acceptance approval/rejection | Bounded local model verdict with fail-closed malformed output. | CONFIRMED / [CHIEF_MACHINE_CONTRACT.md](./CHIEF_MACHINE_CONTRACT.md). | Format/state validation. | Keep with tests. | No | Medium | `/home/openclaw/chief_acceptance_gate.py` | No |
