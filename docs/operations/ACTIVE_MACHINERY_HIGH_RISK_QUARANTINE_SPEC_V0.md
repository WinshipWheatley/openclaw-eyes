...
| `retry_send_demo_dashboard.sh` | `keep_reference_only` | Retired shell retry path previously invoked the demo dashboard sender. | Historical reference only; runtime source removed by dead-code branch. | No service template found; no runtime source remains after retirement branch. | Any future demo must be rebuilt as no-send proof or explicitly approved bounded demo packet. | Restore from git if the master rejects the retirement branch. | Assert retired source path is absent and no gitignore allow-list can reintroduce it silently. | Yes before any future demo send use or restoration. |
...
| `send_demo_dashboard.py` | `keep_reference_only` | Retired one-off demo sender had SMTP/Gmail send/API signals. | Historical reference only; runtime source removed by dead-code branch. | No service template found; retry wrapper is retired with it. | Any future dashboard package must be rebuilt as a read-only/no-send proof artifact or explicitly approved bounded demo packet. | Restore from git if the master rejects the retirement branch. | Assert retired source path is absent and no gitignore allow-list can reintroduce it silently. | Yes before any future send use or restoration. |
