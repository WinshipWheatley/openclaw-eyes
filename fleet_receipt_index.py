...
def _clarification(rows: list[sqlite3.Row]) -> ReceiptResolution:
    ...
    lines = [
        "I found several delivered receipts in this chat. Reply to the original message, "
        "or ask for one of these aliases:"
    ]
    lines.extend(f"- {row['alias']} — {row['what_happened']}" for row in visible_rows)
    if len(rows) > len(visible_rows):
        lines.append("- More delivered records exist; reply to the original message to narrow the lookup.")
    text = "\n".join(lines)
