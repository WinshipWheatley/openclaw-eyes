...
_REPLY_ONLY_AUTHORITY = "Response delivery only; no business-action authority was granted."
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if receipt_request is not None:
        if not is_authorized_user:
            await update.message.reply_text(
                _final_operator_reply(_CONTACT_RECEIPT_LOOKUP_DENIAL, source_request=text)
            )
            return
        ...
        if resolution is not None:
            await update.message.reply_text(
                _final_operator_reply(resolution.text, source_request=text)
            )
            return
    ...
    async def _send_bound_document(document_path: str, caption: str):
        ...
        with _Path(document_path).open("rb") as document:
            return await update.message.reply_document(document=document, caption=safe_caption)
