...
BACKEND_DATA_CONTRACT_SQLITE_IMPLEMENTATION_READINESS = (
    SOURCE_SET_BRIDGE_DIR
    / "backend_data_contract_sqlite_implementation_readiness_20260505.md"
)
BACKEND_DATA_CONTRACT_FIRST_SQLITE_IMPLEMENTATION_PLAN = (
    SOURCE_SET_BRIDGE_DIR
    / "backend_data_contract_first_sqlite_implementation_plan_20260505.md"
)
...
BACKEND_DATA_CONTRACT_MODULE_REQUIRED_TERMS = (
    ...
    "related_schema_contract_surface",
    "can_directly_imply_accepted_truth",
    "semantic_record",
    "semantic_label",
...
BACKEND_DATA_CONTRACT_TEST_REQUIRED_TERMS = (
    ...
    "test_required_schema_contract_surfaces_exist",
    ...
    "test_schema_contract_helpers_are_pure_lookup_and_validation_only",
    "test_required_sqlite_table_concepts_exist",
    "test_sqlite_table_concept_names_normalize_correctly",
    "test_each_sqlite_table_concept_maps_to_expected_schema_surface",
    "test_sqlite_table_concepts_expose_required_conceptual_fields",
    "test_missing_sqlite_table_concept_fields_fail_with_useful_reasons",
    "test_missing_sqlite_table_concept_forbidden_boundaries_fail_closed",
    "test_sqlite_table_concepts_do_not_authorize_forbidden_implementation_terms",
    "test_semantic_records_table_preserves_layer_separation_without_truth_flattening",
    "test_receipt_promotion_and_provenance_tables_remain_distinct",
    "test_sqlite_table_concept_helpers_are_pure_lookup_and_validation_only",
    "SQLite persistence",
    "SQL DDL",
    "migration",
    "persistence",
...
def backend_data_contract_sqlite_implementation_readiness_failures(
    repo_root: Path = REPO_ROOT,
) -> tuple[str, ...]:
    failures: list[str] = []
    readiness_path = (
    ...
    if not readiness_path.is_file():
        return (f"backend SQLite implementation-readiness: missing {readiness_path}",)

    readiness_text = _read_text(readiness_path)
    readiness_normalized = normalize(readiness_text)

    _require_all(
        failures,
        readiness_normalized,
        "backend SQLite implementation-readiness required gates",
        (
            "Backend Data Contract SQLite Implementation Readiness",
            "docs-only SQLite implementation-readiness artifact",
            "does not authorize implementation by itself",
            "first no-runtime SQLite schema-contract code slice",
            "no-runtime",
            "no-persistence",
            "no-file-I/O",
            "no-DB-connection",
            ...
            "Review status",
            "receivables/accountability",
            "semantic accountability records and relationships only",
            "automated harassment",
            "automated sending",
            "collection action",
            "bank access",
            ...
            "CPA action",
            "runtime action",
            "later implementation prompt must be separate",
            "exact-path allowlisted",
            "Stop Conditions",
        ),
    )

    forbidden_authorizing_phrases = (
        "This readiness artifact implements SQLite",
        "This readiness artifact authorizes implementation",
        "This readiness artifact creates SQL DDL",
        "This readiness artifact authorizes SQL DDL execution",
        "This readiness artifact creates migrations",
        "This readiness artifact authorizes persistence",
        "This readiness artifact opens database connections",
        "This readiness artifact authorizes file I/O",
        "This readiness artifact creates API routes",
        "This readiness artifact authorizes ingestion",
        "This readiness artifact authorizes extraction",
        "This readiness artifact authorizes indexing",
        "This readiness artifact authorizes embeddings",
        "This readiness artifact creates fixtures",
        "This readiness artifact authorizes runtime services",
        "This readiness artifact authorizes provider/model calls",
        "This readiness artifact authorizes Hermes",
        "This readiness artifact authorizes MCPs",
        "This readiness artifact authorizes sync",
        "This readiness artifact authorizes source-set generation",
        "This readiness artifact authorizes private-root inspection",
        "This readiness artifact authorizes private-data inspection",
        "SQLite implementation may begin from this readiness artifact alone",
    )
    for phrase in forbidden_authorizing_phrases:
        if phrase in readiness_text:
            failures.append(
                "backend SQLite implementation-readiness: forbidden authorizing "
                f"phrase {phrase!r}"
            )
...
def backend_data_contract_first_sqlite_implementation_plan_failures(
    repo_root: Path = REPO_ROOT,
) -> tuple[str, ...]:
    failures: list[str] = []
    plan_path = (
    ...
    if not plan_path.is_file():
        return (f"backend first SQLite implementation plan: missing {plan_path}",)
    ...
    _require_all(
        failures,
        plan_normalized,
        "backend first SQLite implementation plan required gates",
        (
            "Backend Data Contract First SQLite Implementation Plan",
            "planning-only first SQLite implementation path artifact",
            "does not authorize implementation by itself",
            "future implementation prompt must be separate",
            "exact-path bounded",
            ...
            "tests/test_launch_ladder_static_contract.py",
            "docs/testing/VALIDATION_MAP.md",
            "Python schema-definition module",
            "SQL DDL strings as inert constants only",
            "must not create a migration file",
            "must not execute SQL DDL",
            ...
            "receivables/accountability",
            "semantic accountability records",
            "automated harassment",
            "automated sending",
            "runtime action",
            "private-root inspection",
    ...
    forbidden_authorizing_phrases = (
        "This plan implements SQLite",
        "This plan authorizes SQLite implementation",
        "This plan authorizes sqlite3",
        "This plan opens database connections",
        "This plan authorizes database connections",
        "This plan executes SQL DDL",
        "This plan authorizes SQL DDL execution",
        "This plan creates migrations",
        "This plan authorizes migrations",
        "This plan authorizes persistence",
        "This plan authorizes file I/O runtime behavior",
        "This plan creates API routes",
        "This plan authorizes ingestion",
        "This plan authorizes extraction",
        "This plan authorizes indexing",
        ...
        "This plan creates fixtures",
        "This plan authorizes runtime services",
        "This plan authorizes frontend/app behavior",
        "This plan authorizes app behavior",
        "This plan authorizes provider/model calls",
        "This plan authorizes Hermes",
        "This plan authorizes MCPs",
        "This plan authorizes sync",
    ...
    for phrase in forbidden_authorizing_phrases:
        if phrase in plan_text:
            failures.append(
                "backend first SQLite implementation plan: forbidden authorizing "
                f"phrase {phrase!r}"
            )
...
def check_contract(corpus: ContractCorpus | None = None) -> StaticContractReport:
    ...
    _require_all(
        ...
        (
            ...
            "first SQLite implementation planning bridge",
            "backend_data_contract.py",
            "test_backend_data_contract.py",
            "backend_sqlite_schema.py",
            "test_backend_sqlite_schema.py",
    ...
    failures.extend(backend_data_contract_sqlite_implementation_readiness_failures())
    failures.extend(backend_data_contract_first_sqlite_implementation_plan_failures())
