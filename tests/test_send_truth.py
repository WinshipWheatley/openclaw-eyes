...
class TestCalendarDeleteRouting:
    ...
    def test_handle_routes_exact_delete_request_to_calendar_delete(self, monkeypatch):
        ...
        monkeypatch.setattr(
            cassandra_brain,
            "_log_conversation",
            lambda text, replies, route="llm", **_kwargs: logged.append({"text": text, "replies": replies, "route": route}),
            raising=False,
        )
...
class TestGroundedEmailReviewGate:
    ...
    def _call(
        ...
        *,
        ...
        monkeypatch.setattr(cassandra_brain, "_fetch_payment_verify_context", lambda query, **kwargs: payment_ctx, raising=False)
        monkeypatch.setattr(cassandra_brain, "_fetch_calendar_context", lambda query, **kwargs: "", raising=False)
...
class TestCassandraRouterPolicy:
    ...
    def test_easy_user_reply_uses_small_lane_first(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            ) or ("gemma4:31b", lane or "strong"),
            raising=False,
        )
        monkeypatch.setattr(
            cassandra_brain,
            "ollama_call",
            lambda prompt, timeout=0, model=None, lane=None, task_class=None: model_calls.append(model) or "ok",
            raising=False,
        )
    ...
    def test_outbound_draft_uses_strong_lane(self, monkeypatch):
        ...
        monkeypatch.setattr(
            cassandra_brain,
            "ollama_call",
            lambda prompt, timeout=0, model=None, lane=None, task_class=None: "draft body",
            raising=False,
        )
    ...
    def test_handle_calendar_fallback_logs_llm_deep_without_name_error(self, monkeypatch):
        ...
        monkeypatch.setattr(cassandra_brain, "_should_use_deep", lambda query: True, raising=False)
        monkeypatch.setattr(cassandra_brain, "_use_small_cassandra_reply_model", lambda query: False, raising=False)
        monkeypatch.setattr(cassandra_brain, "_log_conversation", lambda text, replies, route="llm", **_kwargs: logged.append({"text": text, "replies": replies, "route": route}), raising=False)
