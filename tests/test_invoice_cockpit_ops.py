...
def test_clara_draft_resolves_st_annes_registry_contact_for_greeting(tmp_path: Path) -> None:
    ...
    assert "Once you've passed it along to Glenn, could you ask him to send me a quick note when he has it?" in messages[0]
    assert "That way I know it landed and don't have to keep bothering either of you." in messages[0]
...
def test_send_email_test_mode_uses_registry_contact_body(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "Once you've passed it along to Glenn, could you ask him to send me a quick note when he has it?" in calls[0][2]["body"]
    assert "That way I know it landed and don't have to keep bothering either of you." in calls[0][2]["body"]
