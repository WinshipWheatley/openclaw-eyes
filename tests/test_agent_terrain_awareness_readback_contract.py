...
def test_mission_control_guidance_avoids_card_wall_and_live_controls(tmp_path):
    guidance = _build(tmp_path)["mission_control_surface_guidance"]
...
def test_export_writes_json_and_operator_outputs(tmp_path):
    ...
    assert "Agent Terrain Awareness Readback Contract v0" in operator_path.read_text(encoding="utf-8")
