...
def _open_source_scouts(payload: dict) -> dict:
    return payload["open_source_capability_scouts_by_id"]
...
def test_models_exist_with_required_fields_and_vocabularies():
    ...
    assert payload["open_source_capability_scout_schema"]["required_fields"] == list(
        contract.REQUIRED_OPEN_SOURCE_CAPABILITY_SCOUT_FIELDS
    )
    ...
    assert set(payload["open_source_scout_recommendations"]) == set(contract.OPEN_SOURCE_SCOUT_RECOMMENDATIONS)
    assert set(payload["open_source_scout_license_postures"]) == set(contract.OPEN_SOURCE_SCOUT_LICENSE_POSTURES)
...
def test_open_source_capability_scout_models_candidates_and_license_boundaries():
    payload = _build()
    scouts = _open_source_scouts(payload)

    scout = scouts["safe_preview_provider_open_source_scout_v0"]
    assert set(contract.REQUIRED_OPEN_SOURCE_CAPABILITY_SCOUT_FIELDS) <= set(scout)
    assert scout["capability_needed"] == "safe preview provider for invoice, legal, discovery, and untrusted document artifacts"
    assert scout["recommendation"] == "WRAP"
    assert "Dangerzone" in scout["license_summary"]
    assert scout["code_can_be_used_directly"] is False
    assert scout["recommended_next_step"].startswith("Keep current Mac Quick Look path")
    assert payload["machine_proof"]["open_source_capability_scout_model_present"] is True
    assert payload["machine_proof"]["open_source_scout_grants_dependency_authority"] is False
    assert payload["machine_proof"]["open_source_scout_preserves_license_attribution"] is True
    assert payload["machine_proof"]["open_source_scout_flags_copyleft"] is True


def test_open_source_candidate_recommendations_cover_reuse_wrap_adapt_and_avoid():
    scout = _open_source_scouts(_build())["safe_preview_provider_open_source_scout_v0"]
    candidates = {candidate["project_ref"]: candidate for candidate in scout["candidate_projects"]}

    assert candidates["mac_quicklook_client_preview"]["recommendation"] == "REUSE"
    assert candidates["dangerzone_backend_sanitizer"]["recommendation"] == "WRAP"
    assert candidates["dangerzone_backend_sanitizer"]["license_posture"] == "COPYLEFT_REVIEW_REQUIRED"
    assert candidates["dangerzone_backend_sanitizer"]["attribution_required"] is True
    assert candidates["libreoffice_headless_converter"]["recommendation"] == "ADAPT"
    assert candidates["onlyoffice_document_server"]["recommendation"] == "AVOID"
    assert candidates["onlyoffice_document_server"]["ideas_tests_docs_only"] is True
    for candidate in candidates.values():
        assert candidate["license_posture"] in contract.OPEN_SOURCE_SCOUT_LICENSE_POSTURES
        assert candidate["recommendation"] in contract.OPEN_SOURCE_SCOUT_RECOMMENDATIONS
        assert candidate["license"]
        assert candidate["security_risks"]
        assert candidate["integration_mode"]
...
def test_all_authority_flags_false_and_no_external_authority():
    ...
    assert payload["authority_boundary"]["open_source_package_download_allowed"] is False
    assert payload["authority_boundary"]["dependency_install_allowed"] is False
    assert payload["authority_boundary"]["license_evasion_allowed"] is False
    assert payload["authority_boundary"]["code_laundering_allowed"] is False
...
def test_exporter_writes_json_and_eliwinship_operator_markdown(tmp_path):
    ...
    assert payload["machine_proof"]["open_source_capability_scout_count"] == 1
    assert payload["machine_proof"]["capital_hilton_coupa_po_bottleneck_present"] is True
    assert payload["machine_proof"]["all_current_authority_flags_false"] is True
    assert "ELIWINSHIP Summary" in operator
    assert "Manual forever is not the goal" in operator
    assert "Open Source Capability Scout" in operator
