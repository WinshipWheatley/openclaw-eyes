...
import openclaw_chatgpt55_adapter as adapter
...
def _provider_with_result(result_factory, capture: dict | None = None):
    def fake_provider(*, request_payload: dict, request_body: dict, model_label: str, timeout_seconds: int) -> dict:
        ...
        return {"id": "resp_fake", "status": "completed", "output_text": output_text}
...
def test_adapter_disabled_blocks_before_provider(monkeypatch):
    ...
    availability = adapter.is_live_chatgpt55_available()
    with pytest.raises(adapter.ChatGPT55AdapterError) as exc:
        adapter.call_chatgpt55_data_room_advisory(_package(), "hello", "", provider=provider, created_at_utc=FIXED_NOW)
...
def test_missing_credential_blocks_without_printing_secret(monkeypatch, capsys):
    ...
    availability = adapter.is_live_chatgpt55_available()
    with pytest.raises(adapter.ChatGPT55AdapterError) as exc:
        adapter.call_chatgpt55_data_room_advisory(
            _package(),
            "hello",
            "",
...
def test_fake_provider_valid_schema_returns_structured_result_and_no_tools(monkeypatch):
    ...
    result = adapter.call_chatgpt55_data_room_advisory(
        _package(),
        "Acknowledge readiness for helping with this Data Room form. Do not answer the form yet.",
        "",
    ...
    assert capture["request_body"]["text"]["format"]["type"] == "json_schema"
    assert capture["request_body"]["text"]["format"]["strict"] is True
    assert capture["request_body"]["model"] == "gpt-5.5"
...
def test_facts_used_must_reference_supplied_package_only(monkeypatch):
    monkeypatch.setenv("OPENCLAW_ENABLE_LIVE_CHATGPT55", "1")
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test-redacted")

    def unsafe(request):
        result = _safe_result(request)
        result["facts_used"] = ["privacy:payment_policy", "unknown:outside_fact"]
        return result
    ...
    assert "facts_used_outside_package" in exc.value.validation["errors"]
