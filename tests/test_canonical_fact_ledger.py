...
def test_rejects_empty_fact_text():
    with pytest.raises(ValueError, match="fact_text cannot be empty"):
        record_canonical_fact("f2", "doc.md", "Overview", "abc", "", "public_canonical", [], DB_PATH)

def test_rejects_missing_provenance():
    with pytest.raises(ValueError, match="Missing mandatory provenance fields"):
        record_canonical_fact("f3", "", "Overview", "abc", "fact", "public_canonical", [], DB_PATH)

def test_rejects_unsafe_sensitivity():
    with pytest.raises(ValueError, match="Invalid sensitivity_class"):
        record_canonical_fact("f4", "d.md", "O", "a", "fact", "SUPER_SECRET", [], DB_PATH)

def test_rejects_pii():
    with pytest.raises(ValueError, match="PII detected in fact_text"):
        record_canonical_fact("f5", "d.md", "O", "a", "Contact me at bob@example.com", "public_canonical", [], DB_PATH)
    with pytest.raises(ValueError, match="PII detected in fact_text"):
        record_canonical_fact("f6", "d.md", "O", "a", "My SSN is 123-45-6789", "public_canonical", [], DB_PATH)
    with pytest.raises(ValueError, match="PII detected in fact_text"):
        record_canonical_fact("f7", "d.md", "O", "a", "Call 555-555-5555", "public_canonical", [], DB_PATH)
...
def test_data_integrity():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT content_hash, sensitivity_class FROM canonical_facts WHERE fact_id = 'f8'")
    row = cursor.fetchone()
    conn.close()
