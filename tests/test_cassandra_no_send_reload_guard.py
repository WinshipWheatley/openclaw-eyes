...
def test_no_send_reload_guard_env_truthy(monkeypatch):
    ...
    monkeypatch.delenv(guard.MODE_ENV_VAR, raising=False)
    monkeypatch.delenv(guard.ALT_MODE_ENV_VAR, raising=False)
    assert guard.is_no_send_reload_guard_enabled() is False
    assert guard.should_quiesce_send_capable_service() is False
...
def test_status_dry_run_mode_advances_without_removing_emergency_guard(monkeypatch):
    import cassandra_no_send_reload_guard as guard
...
def test_watcher_first_tick_quiesces_all_send_capable_paths(monkeypatch):
    ...
    monkeypatch.setattr(watcher, "is_status_dry_run_enabled", lambda: False, raising=False)
...
def test_watcher_status_dry_run_inspects_without_outbound_paths(monkeypatch):
    monkeypatch.setenv("CASSANDRA_NO_SEND_RELOAD_GUARD", "1")
    monkeypatch.setenv("CASSANDRA_SEND_CAPABLE_MODE", "status_dry_run")
    import cassandra_watcher as watcher
    ...
    monkeypatch.setattr(
        ...
        lambda: observed.setdefault("status", {"pending_followups": {"pending_count": 2}, "future_actions": {"pending_count": 1, "due_count": 1}}),
        raising=False,
    )
    monkeypatch.setattr(watcher, "_restart_if_sources_changed", _fail("source reload"), raising=False)
    monkeypatch.setattr(watcher, "process_pending_followups", _fail("pending followups"), raising=False)
...
def test_briefing_scheduler_first_tick_quiesces_all_delivery_paths(monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "is_status_dry_run_enabled", lambda: False, raising=False)
...
def test_briefing_scheduler_status_dry_run_inspects_without_delivery(monkeypatch):
    monkeypatch.setenv("CASSANDRA_NO_SEND_RELOAD_GUARD", "1")
    monkeypatch.setenv("CASSANDRA_SEND_CAPABLE_MODE", "status_dry_run")
    import cassandra_briefing_scheduler as scheduler
    ...
    monkeypatch.setattr(
        scheduler,
        "build_briefing_scheduler_status",
        lambda: observed.setdefault("status", {"briefing_schedule": {"due_slots": ["morning"], "pending_count": 1}}),
        raising=False,
    )
    monkeypatch.setattr(scheduler, "_restart_if_sources_changed", _fail("source reload"), raising=False)
    monkeypatch.setattr(scheduler, "due_slots", _fail("due slot lookup"), raising=False)
