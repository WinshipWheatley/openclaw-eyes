...
import lm2_room_backed_worker_structured_output_retry as retry
...
def _fixture_root(tmp_path: Path) -> Path:
    ...
    _write_json(root / "lm2_structured_output_retry_operator_approval.json", _structured_approval())
    _write_json(root / "lm2_structured_output_retry_approval_packet.json", {"status": "LM2_STRUCTURED_OUTPUT_RETRY_APPROVAL_PACKET_READY"})
    _write_json(root / "lm2_room_backed_worker_pilot_postmortem.json", {"status": "LM2_ROOM_BACKED_WORKER_PILOT_POSTMORTEM_READY"})
    _write_json(root / retry.prior_pilot.JSON_EXPORT_NAME, {"status": retry.prior_pilot.READY_STATUS})
...
def _safe_invoker(prompt: str, schema: dict) -> dict:
    assert schema["type"] == "object"
    assert set(schema["required"]) == set(retry.schema_adapter.STRICT_DRAFT_FIELDS)
    ...
    return {
        ...
        "runtime_ref": "ollama",
        "model_name": "qwen3:8b-q4_K_M",
        "method": retry.STRUCTURED_OUTPUT_METHOD,
...
def _bad_invoker(prompt: str, schema: dict) -> dict:
    return {
        "attempted": True,
        "runtime_ref": "ollama",
        "model_name": "qwen3:8b-q4_K_M",
...
def _unenforced_invoker(prompt: str, schema: dict) -> dict:
    return {
        "attempted": True,
        "runtime_ref": "ollama",
        "model_name": "qwen3:8b-q4_K_M",
        ...
        "structured_output_unavailable_reason": "format_schema_not_supported",
        "temperature": 0,
        "returncode": 1,
        "http_status": 400,
        "stdout": "",
        "stderr": "format schema not supported",
        "timed_out": False,
    }


def _run(tmp_path: Path, invoker=_safe_invoker) -> dict:
    return retry.run_structured_output_retry(
...
def test_invocation_attempt_count_is_exactly_one(tmp_path):
    ...
    def invoker(prompt: str, schema: dict) -> dict:
        calls.append((prompt, schema))
        return _safe_invoker(prompt, schema)
...
def test_structured_output_retry_approval_required_and_marked_used(tmp_path):
    ...
    assert read_model["machine_proof"]["structured_output_retry_approval_required_and_marked_used"] is True
...
def test_structured_output_boundary_receipt_recorded(tmp_path):
    read_model = _run(tmp_path)
    refs = {row["receipt_ref"] for row in read_model["receipts"]}

    assert "structured_output_boundary_receipt" in refs
    assert read_model["structured_output_boundary"]["format_schema_sent"] is True
    assert read_model["structured_output_boundary"]["structured_output_enforced"] is True
    assert read_model["machine_proof"]["structured_output_boundary_receipt_recorded"] is True
...
def test_fallback_works_if_adapter_or_verifier_fails(tmp_path):
    ...
    assert read_model["status"] == retry.READY_STATUS


def test_unenforced_structured_output_records_not_ready(tmp_path):
    read_model = _run(tmp_path, invoker=_unenforced_invoker)

    assert read_model["status"] == retry.NOT_READY_STATUS
    assert read_model["approval_usage"]["approval_used"] is True
    assert read_model["structured_output_boundary"]["structured_output_enforced"] is False
    assert read_model["structured_output_boundary"]["structured_output_unavailable_reason"] == "format_schema_not_supported"
    assert any(row["receipt_ref"] == "fallback_receipt" for row in read_model["receipts"])


def test_sqlite_receipts_recorded(tmp_path):
    sqlite_path = tmp_path / "retry.sqlite"
    read_model = retry.run_structured_output_retry(
        read_model_root=_fixture_root(tmp_path),
        generated_at=FIXED_NOW,
    ...
    with sqlite3.connect(sqlite_path) as conn:
        row_count = conn.execute("SELECT COUNT(*) FROM lm2_room_backed_worker_structured_output_retry_receipts").fetchone()[0]
...
def test_export_json_bridge_equality_latest_and_wiki(tmp_path):
    result = retry.export_structured_output_retry(
        read_model_root=_fixture_root(tmp_path),
        export_root=tmp_path / "read_models",
        bridge_root=tmp_path / "bridge",
        wiki_path=tmp_path / "LM2 Room Backed Worker Structured Output Retry.md",
        sqlite_path=tmp_path / "retry.sqlite",
        generated_at=FIXED_NOW,
        invoker=_safe_invoker,
        invoke_model=True,
    ...
    assert result["structured_output_enforced"] == "true"
    assert local == bridge
    ...
    assert wiki.startswith("# LM2 Room Backed Worker Structured Output Retry")
