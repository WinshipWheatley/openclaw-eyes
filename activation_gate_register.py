...
def _register_gap_capabilities(last_verified_at: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _capability(
            ...
            current_state_if_verifiable=_state(
                "standalone activation record for packet-source sqlite/hybrid flip; related continuity context is preserved separately",
                code_default="flat unless OPENCLAW_PACKET_SOURCE selects sqlite or hybrid",
                production="unknown_or_related_live_context_only; this task did not inspect live services",
                audit_report="OPUS_ACTIVATION_DECISIONS queues packet-source SQLite flip for canary",
            ),
            source_files=["maestro_context_packet.py", "activation_gate_register.py"],
            tests=["tests/test_activation_gate_register.py"],
            audits=[decisions, gaps_audit],
            canary_status="queued_for_hybrid_canary",
            risk_level="medium",
            owner="Opus",
            gate_stage="canary",
            enabled_by="operator-approved hybrid/sqlite packet-source canary with content-diff receipt",
            disabled_by="flat default or unset OPENCLAW_PACKET_SOURCE",
            rollback_note="set OPENCLAW_PACKET_SOURCE=flat or unset it through approved service configuration",
            next_required_step="run a supervised hybrid packet content-diff canary before production flip",
            activation_allowed_now=False,
            operator_approval_required=True,
            reason_if_off="packet content changes need a canary and rollback receipt before activation",
            evidence_refs=[
...
def _capability_templates(last_verified_at: str) -> list[dict[str, Any]]:
    capabilities = [
        ...
        _capability(
            ...
            default_state="default-off",
            current_state_if_verifiable=_state(
                "deterministic task-package materialization is built as a default-off candidate for Polish Loop blocker #3",
                code_default="off unless OPENCLAW_POLISH_LOOP_TASK_PACKAGE_V1 is explicitly truthy",
                production="not_enabled_by_this_task; production activation prohibited",
                audit_report="POLISH-BLOCKER-003 audit identified sparse/stale task.md materialization as a factory blocker",
            ),
            source_files=[
                "polish_loop/worker_runtime.py",
                "polish_loop/orchestrator.py",
            ...
            owner="Opus",
            gate_stage="intentionally_off",
            enabled_by="explicit OPENCLAW_POLISH_LOOP_TASK_PACKAGE_V1 only after Opus review, synthetic tests, canary, rollback proof, and operator-approved runtime scope",
            disabled_by="default-off flag; legacy task.md/directive materialization remains unchanged when unset",
            rollback_note="unset OPENCLAW_POLISH_LOOP_TASK_PACKAGE_V1 or set it to 0; legacy task.md/directive materialization remains available",
            next_required_step="Opus review of blocker #3 implementation, then a separate synthetic/canary authorization task if factory switch criteria otherwise pass",
            activation_allowed_now=False,
            operator_approval_required=True,
            reason_if_off="changes builder input materialization and Polish Loop remains NOT_READY; canary and Opus/operator approval are required",
            evidence_refs=[
