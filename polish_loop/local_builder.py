...
WORK_DIR = Path("/home/openclaw")
LOOP_DIR = WORK_DIR / "polish_loop"
PC_OUTPUT = LOOP_DIR / "current" / "pc_output.md"
...
# Commands allowed by run_command. Anything else is refused.
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Allowed: python3 -m pytest, python3 -c, cat, head, tail, ls, wc, grep, find, git log/diff/status/show. "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "The path MUST be /home/openclaw/polish_loop/current/pc_output.md. "
            ...
            "parameters": {
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Must be /home/openclaw/polish_loop/current/pc_output.md",
                    },
                    "content": {
                        "type": "string",
...
def _exec_run_command(args: dict) -> str:
    ...
    if not any(cmd.startswith(prefix) for prefix in SAFE_COMMAND_PREFIXES):
        return (
            f"ERROR: command not allowed. Safe prefixes: "
            f"{', '.join(SAFE_COMMAND_PREFIXES[:6])}..."
        )
...
def _exec_write_file(args: dict) -> str:
    ...
    if not p.is_absolute():
        p = WORK_DIR / p
    if p not in WRITABLE_PATHS and not str(p).startswith(str(WORK_DIR / "tests")):
    ...
    if p.exists() and p not in WRITABLE_PATHS:
        ...
        if existing_size > 200 and len(content) < existing_size * 0.5:
            return (
                ...
                f"half the existing file ({existing_size} bytes). Use sed -i for small edits "
                f"instead of rewriting the entire file."
            )
    try:
...
def _exec_insert_block(args: dict) -> str:
    ...
    if not p.is_absolute():
        p = WORK_DIR / p
    if not str(p).startswith(str(WORK_DIR / "tests")):
...
def _claim_ledger_task(db_path: str, task_id: str, owner: str, lease_seconds: int):
    try:
        from control_plane import ControlPlaneLedger
    except ImportError as exc:
        _log(f"ledger unavailable: {exc}")
        return None
    ledger = ControlPlaneLedger(db_path)
    lease = ledger.claim_task(task_id, owner=owner, lease_seconds=lease_seconds)
    if lease is None:
        _log(f"ledger claim refused for task={task_id} owner={owner}")
        return None
    _log(f"ledger claim ok task={task_id} nonce={lease.lease_nonce[:8]} attempt={lease.attempt_no}")
    return ledger, lease


def _finish_ledger_task(ledger, lease, rc: int) -> None:
    try:
        if rc == 0 and PC_OUTPUT.exists():
            ...
            _log(f"ledger candidate submitted task={lease.task_id}")
        else:
            ledger.record_failure(
                lease.task_id,
                owner=lease.owner,
                lease_nonce=lease.lease_nonce,
                failure_fingerprint=f"local_builder_exit_{rc}",
            )
            _log(f"ledger failure recorded task={lease.task_id} rc={rc}")
    except Exception as exc:
        _log(f"ledger finish failed task={lease.task_id}: {exc}")
...
def main():
    ...
    parser.add_argument("--ledger-db", help="Optional Phase-C ledger path")
    parser.add_argument("--ledger-task-id", help="Optional Phase-C task id to claim before running")
    parser.add_argument("--lease-owner", default="local_builder", help="Ledger lease owner")
    parser.add_argument("--lease-seconds", type=int, default=3600, help="Ledger lease duration")
    args = parser.parse_args()

    PC_OUTPUT.parent.mkdir(parents=True, exist_ok=True)

    ledger_claim = None
    if args.ledger_db or args.ledger_task_id:
        if not args.ledger_db or not args.ledger_task_id:
            _log("ABORT: --ledger-db and --ledger-task-id must be supplied together")
            sys.exit(1)
    ...
    if ledger_claim is not None:
        ...
        _finish_ledger_task(ledger, lease, rc)
