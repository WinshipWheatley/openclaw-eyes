"""One-time room-backed LM2 worker pilot V0.

Runs one approved local Ollama worker attempt for Finance / Capital Hilton.
The model receives only a bounded room-backed package with redacted,
freshness-gated proof and summaries. Publication is gated by the strict schema
adapter and deterministic verifier; failures publish a safe fallback.
"""
...
DEFAULT_WIKI_PATH = Path("generated/wiki/openclaw/LM2 Room Backed Worker One Time Pilot.md")
DEFAULT_SQLITE_PATH = Path("generated/system_knowledge/lm2_room_backed_worker_one_time_pilot.sqlite")

SCHEMA_VERSION = "lm2_room_backed_worker_one_time_pilot_v0"
READ_MODEL_ID = "lm2_room_backed_worker_one_time_pilot"
JSON_EXPORT_NAME = f"{READ_MODEL_ID}.json"
READY_STATUS = "LM2_ROOM_BACKED_WORKER_ONE_TIME_PILOT_READY"
NOT_READY_STATUS = "LM2_ROOM_BACKED_WORKER_ONE_TIME_PILOT_NOT_READY"
...
CANDIDATE_SOURCE = "lm2_room_backed_worker_one_time_pilot"
SOURCE_REQUEST_ID = "lm2_room_backed_worker_one_time_pilot_finance_capital_hilton_payment_watch"

PRECONDITIONS = {
    "lm2_room_backed_worker_pilot_operator_approval": {
        "filename": "lm2_room_backed_worker_pilot_operator_approval.json",
        "accepted_statuses": ("LM2_ROOM_BACKED_WORKER_PILOT_OPERATOR_APPROVAL_READY",),
    },
...
def build_room_backed_package() -> dict[str, Any]:
    return boundary.build_room_backed_package()


def approval_scope_matches(read_model_root: Path = DEFAULT_READ_MODEL_ROOT) -> bool:
    root = _rooted(read_model_root)
    approval = _load_json(root / "lm2_room_backed_worker_pilot_operator_approval.json")
    scope = approval.get("approval_scope") if isinstance(approval.get("approval_scope"), Mapping) else {}
    return (
        approval.get("status") == "LM2_ROOM_BACKED_WORKER_PILOT_OPERATOR_APPROVAL_READY"
        and approval.get("operator_decision") == "approve_one_time_room_backed_lm2_worker_pilot"
...
def approval_unused(read_model_root: Path = DEFAULT_READ_MODEL_ROOT) -> bool:
    ...
    return not (prior.get("status") == READY_STATUS and usage.get("approval_used") is True)


def package_matches_scope(package: Mapping[str, Any]) -> bool:
    return (
        package.get("worker_class") == WORKER_CLASS
        and package.get("runtime") == RUNTIME_REF
        and package.get("model") == MODEL_NAME
        and package.get("lane") == PILOT_LANE
        and package.get("objective") == OBJECTIVE_REF
        and package.get("question") == QUESTION
        and package.get("mode") == MODE
        and package.get("room_backed_package_required") is True
        and package.get("project_room_ready_required") is True
    )


def project_room_gate(package: Mapping[str, Any], read_model_root: Path = DEFAULT_READ_MODEL_ROOT) -> dict[str, Any]:
    contract = _load_json(_rooted(read_model_root) / "project_room_sourceset_contract.json")
    room_id = str(package.get("project_room_id") or "")
    source_inventory_ref = str(package.get("source_inventory_ref") or "")
    duplicate_report_ref = str(package.get("duplicate_report_ref") or "")
    room = next((row for row in contract.get("project_rooms") or [] if row.get("project_room_id") == room_id), {})
    inventory_rows = [
        row
        for row in contract.get("source_inventory") or []
        if row.get("project_room_id") == room_id and row.get("source_inventory_ref") == source_inventory_ref
    ]
    conflict_rows = [row for row in contract.get("conflict_log") or [] if row.get("project_room_id") == room_id]
    duplicate_rows = [
        row
        for row in contract.get("duplicate_version_report") or []
        if row.get("project_room_id") == room_id and row.get("version_family_ref") == duplicate_report_ref
    ]
    missing_context_rows = [row for row in contract.get("missing_context_list") or [] if row.get("project_room_id") == room_id]

    unresolved_critical_conflict = any(row.get("unresolved") is True for row in conflict_rows)
    source_inventory_exists = bool(inventory_rows)
    current_inventory_exists = any(row.get("apparent_authority") == "current_receipts_and_proof" for row in inventory_rows)
    duplicate_report_exists = bool(duplicate_rows)
    duplicate_current_truth_ok = any(row.get("likely_current_source_ref") == "source:finance_payment_watch_state" for row in duplicate_rows)
    stale_source_as_current_truth_absent = all(
        not (row.get("source_ref") == "source:finance_generated_summary" and row.get("apparent_authority") == "current_receipts_and_proof")
        for row in inventory_rows
    )
    missing_context_blocks_supported_answer = any(
        "paid claim" in [str(item) for item in room.get("missing_context_blocks") or []]
        and "mark paid" not in str(row.get("safe_wording_if_unresolved") or "").lower()
        for row in missing_context_rows
    )
    room_ready = (
        bool(room)
        and room.get("synthesis_allowed") is True
        and room.get("inventory_gate") == "passed_with_limited_scope"
        and source_inventory_exists
        and current_inventory_exists
        and not unresolved_critical_conflict
        and not missing_context_blocks_supported_answer
        and duplicate_report_exists
        and duplicate_current_truth_ok
        and stale_source_as_current_truth_absent
    )
    return {
        "project_room_id": room_id,
        "project_room_ready": room_ready,
        "project_room_record_exists": bool(room),
        "source_inventory_ref": source_inventory_ref,
        "source_inventory_exists": source_inventory_exists,
        "current_inventory_exists": current_inventory_exists,
        ...
        "missing_context_blocks_supported_answer": missing_context_blocks_supported_answer,
        "duplicate_report_ref": duplicate_report_ref,
        "duplicate_report_exists": duplicate_report_exists,
        "duplicate_current_truth_ok": duplicate_current_truth_ok,
        "stale_source_as_current_truth_absent": stale_source_as_current_truth_absent,
        "duplicate_versions_as_equal_evidence": False,
        "missing_context_used_as_invention_permission": False,
        "allowed_scope": room.get("synthesis_scope") or "",
        "blocked_next_steps": list(room.get("blocked_next_steps") or []),
        "safe_wording_if_unresolved": [str(row.get("safe_wording_if_unresolved") or "") for row in missing_context_rows],
    }


def build_redacted_proof_bundle(read_model_root: Path = DEFAULT_READ_MODEL_ROOT) -> dict[str, Any]:
    return bundles.build_redacted_proof_bundle(SCENARIO_ID, read_model_root=read_model_root)


def forbidden_fields_absent(redacted_bundle: Mapping[str, Any]) -> bool:
...
def freshness_gate_check(redacted_bundle: Mapping[str, Any]) -> dict[str, Any]:
    redaction_errors = bundles.validate_redacted_proof_bundle(redacted_bundle)
    allowed = (
        redacted_bundle.get("allowed_for_lm_bundle") is True
        and redacted_bundle.get("freshness_state") == "current"
        and redacted_bundle.get("confidence_class") == "receipt_backed"
        and redacted_bundle.get("trusted_current") is True
        and not redaction_errors
        and forbidden_fields_absent(redacted_bundle)
    )
    return {
        "freshness_gate_checked": True,
        "freshness_allowed": allowed,
        "allowed_for_lm_bundle": redacted_bundle.get("allowed_for_lm_bundle") is True,
        "freshness_state": redacted_bundle.get("freshness_state"),
        "confidence_class": redacted_bundle.get("confidence_class"),
        "trusted_current": redacted_bundle.get("trusted_current") is True,
        "redaction_validation_errors": redaction_errors,
        "forbidden_fields_absent": forbidden_fields_absent(redacted_bundle),
    }


def verifier_bundle_for_room(redacted_bundle: Mapping[str, Any]) -> dict[str, Any]:
    verifier_bundle = bundles.redacted_bundle_for_verifier(redacted_bundle)
    known = {str(row.get("fact_id")) for row in verifier_bundle.get("known_facts") or [] if isinstance(row, Mapping)}
    aliases = [
        {
            "fact_id": "processor_processing",
            "text": "Coupa is processing.",
            "source_refs": ["generated/read_models/proof_bundle_freshness_trace_status.json"],
        },
        {
            "fact_id": "paid_false",
            "text": "Paid state is false because payment evidence is missing.",
            "source_refs": ["generated/read_models/proof_bundle_freshness_trace_status.json"],
        },
    ]
    verifier_bundle["known_facts"] = list(verifier_bundle.get("known_facts") or []) + [
        row for row in aliases if row["fact_id"] not in known
    ]
    controls = list(verifier_bundle.get("allowed_response_controls") or [])
    if not any(control.get("label") == "attach_proof" for control in controls if isinstance(control, Mapping)):
        controls.append(
            {
                "label": "attach_proof",
    ...
    verifier_bundle["allowed_response_controls"] = controls
    return verifier_bundle


def lm_visible_room_package(package: Mapping[str, Any], redacted_bundle: Mapping[str, Any]) -> dict[str, Any]:
    lm_input = redacted_bundle.get("lm_input") if isinstance(redacted_bundle.get("lm_input"), Mapping) else {}
    return {
        "redacted_freshness_gated_proof_bundle": {key: lm_input.get(key) for key in sorted(lm_input)},
        "current_lane_summary": package.get("current_lane_summary"),
        "source_inventory_summary": package.get("source_inventory_summary"),
        "missing_context_summary": package.get("missing_context_summary"),
        "decision_trace_summary": package.get("decision_trace_summary"),
        "proof_meter_labels": list(package.get("proof_meter_labels") or []),
        "allowed_controls": list(package.get("allowed_controls") or []),
        "blocked_action_summaries": list(package.get("blocked_action_summaries") or []),
        "required_json_response_schema": package.get("required_json_response_schema"),
        "one_valid_json_example": package.get("one_valid_json_example"),
        "stop_conditions": list(package.get("stop_conditions") or []),
    }


def build_prompt(lm_package: Mapping[str, Any]) -> str:
    payload = {
        "worker_class": WORKER_CLASS,
        "task": "Return one JSON proof-to-response candidate for the operator question.",
        "question": QUESTION,
        "strict_rules": [
            "Return JSON only.",
...
def invoke_ollama_once(prompt: str, *, timeout_seconds: int = 180) -> dict[str, Any]:
    started_at = utc_now()
    try:
        ...
        return {
            ...
            "stderr": str(exc.stderr or "ollama invocation timed out"),
            "timed_out": True,
        }
...
def safe_room_backed_fallback_candidate(verifier_bundle: Mapping[str, Any], *, reason: str) -> dict[str, Any]:
    return {
        "response_id": "lm2_room_backed_worker:fallback:finance_capital_hilton_payment_watch",
...
def _receipt(
    ...
    *,
    ...
    return {
        "receipt_id": f"lm2_room_backed_worker_pilot:{receipt_ref}",
...
def action_flags() -> dict[str, bool]:
    return {
        ...
        "additional_worker_spawn_performed": False,
        "lm2_shell_command_performed": False,
        "lm2_file_system_mutation_performed": False,
    ...
    *,
    ...
    receipts = [
        _receipt(
            ...
            phase="before_future_attempt",
            source_ref="generated/read_models/lm2_room_backed_worker_pilot_operator_approval.json",
            proof_summary="Operator approved one future room-backed LM2 worker attempt.",
        ),
        _receipt(
            "room_backed_package_receipt",
            receipt_status="present",
        ...
        _receipt(
            ...
            proof_summary="Only redacted freshness-gated proof bundle fields were included.",
        ...
        _receipt(
            ...
            receipt_status="present",
            created_at=generated_at,
            phase="after_future_attempt",
            proof_summary="The bounded LM2 worker attempt started.",
        ...
        _receipt(
            ...
            created_at=generated_at,
            phase="after_future_attempt",
            proof_summary="Exactly one local Ollama model invocation attempt was recorded.",
            payload={
        ...
        _receipt(
            ...
            created_at=generated_at,
            phase="after_future_attempt",
            proof_summary="The bounded LM2 worker attempt stopped after one model attempt.",
        ...
        _receipt(
            ...
            created_at=generated_at,
            phase="after_future_attempt",
            proof_summary=f"Schema adapter then verifier result: {'pass' if verifier_passed else 'fail'}.",
            payload={
    ...
    if verifier_passed:
        ...
        receipts.append(
            _receipt(
                ...
                created_at=generated_at,
                phase="after_future_attempt",
                proof_summary="Adapter or verifier failed; safe fallback was published.",
...
def sqlite_schema() -> str:
    return """
CREATE TABLE IF NOT EXISTS lm2_room_backed_worker_pilot_receipts (
...
CREATE INDEX IF NOT EXISTS idx_lm2_room_backed_worker_pilot_receipts_ref
ON lm2_room_backed_worker_pilot_receipts(receipt_ref);
"""
...
def write_sqlite(receipts: list[Mapping[str, Any]], sqlite_path: Path = DEFAULT_SQLITE_PATH) -> int:
    ...
    with sqlite3.connect(path) as conn:
        ...
        conn.execute("DELETE FROM lm2_room_backed_worker_pilot_receipts")
        for row in receipts:
            conn.execute(
                """
INSERT INTO lm2_room_backed_worker_pilot_receipts (
...
def sqlite_row_count(sqlite_path: Path = DEFAULT_SQLITE_PATH) -> int:
    ...
    if not path.exists():
        return 0
    with sqlite3.connect(path) as conn:
        row = conn.execute("SELECT COUNT(*) FROM lm2_room_backed_worker_pilot_receipts").fetchone()
...
def build_latest_read_model(
    ...
    *,
    ...
    latest = {
        ...
        "selected_action_id": "lm2_room_backed_worker_one_time_pilot",
        "candidate_source": CANDIDATE_SOURCE,
...
def run_one_time_pilot(
    *,
    ...
    redacted_bundle = build_redacted_proof_bundle(read_model_root)
    verifier_bundle = verifier_bundle_for_room(redacted_bundle)
    room_gate = project_room_gate(package, read_model_root)
    freshness_gate = freshness_gate_check(redacted_bundle)
    ...
    lm_package = lm_visible_room_package(package, redacted_bundle)
    prompt = build_prompt(lm_package)
    if not invoke_model:
        invocation_result = {
            "attempted": False,
            "started_at": "",
    ...
    required_receipts_present = set(REQUIRED_RECEIPT_REFS) <= {str(row.get("receipt_ref")) for row in receipts} and (
        any(row.get("receipt_ref") == "published_response_hash_receipt" for row in receipts)
        or any(row.get("receipt_ref") == "fallback_receipt" for row in receipts)
    )
    ...
    payload: dict[str, Any] = {
        ...
        "generated_at": generated_at,
        "purpose": "Run one approved room-backed LM2 worker attempt for Finance / Capital Hilton payment-watch proof-to-response.",
        "operator_decision": "approve_one_time_room_backed_lm2_worker_pilot",
        "approval_usage": {
            "approval_required": True,
            "approval_ref": "generated/read_models/lm2_room_backed_worker_pilot_operator_approval.json",
            "approval_matched": approval_matched,
            "approval_unused_before_run": unused_before,
            "approval_used": status_ready,
            "approval_used_at": generated_at if status_ready else "",
            "approval_consumed_by": READ_MODEL_ID if status_ready else "",
        },
        "pilot_scope": {
            "attempt_limit": 1,
    ...
    payload["machine_proof"] = {
        ...
        "approval_required_and_marked_used": payload["approval_usage"]["approval_required"] is True and payload["approval_usage"]["approval_used"] is True,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the one-time room-backed LM2 worker pilot.")
