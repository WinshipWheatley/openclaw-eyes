...
GENERATOR_SOURCE="/home/openclaw/dashboard_gen.py"
GENERATOR_LOG="/mnt/c/OpenClaw/logs/dashboard_gen.out"
NOW_FILE="/home/openclaw/mac_eyes/Winship/Right now.md"
...
generator_pid() {
    pgrep -fo "$GENERATOR_PATTERN" || true
}
...
generator_code_stale() {
    local pid="$1"
    [ -n "$pid" ] || return 1
    [ -e "/proc/$pid" ] || return 1
    [ -f "$GENERATOR_SOURCE" ] || return 1

    local proc_started source_mtime
    proc_started=$(stat -c %Y "/proc/$pid" 2>/dev/null || echo 0)
    source_mtime=$(stat -c %Y "$GENERATOR_SOURCE" 2>/dev/null || echo 0)
    [ "$source_mtime" -gt "$proc_started" ]
}
...
while true; do
    ...
    if [ "$count" -eq 0 ]; then
        ...
        restart_generator
    elif generator_code_stale "$pid"; then
        log "generator code updated on disk; restarting"
        restart_generator
